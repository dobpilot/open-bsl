...
// только для examples/bench.rs (cargo run --release --example bench -p
// bsl-vm), НЕ для cargo test.
//
// ОТКЛОНЕНИЕ ОТ БРИФА: попросили 100000 итераций Advance. Измерено на этом
// движке (cargo run --release): 1000 итераций падают через ~98с с
// "превышен предел масштаба" (MAX_SCALE в bsl-number), а даже 600
// (без ошибки) занимают 79с при росте где-то между квадратичным и
// кубическим от числа итераций — 100000 не то что "почти бесконечны", а
// буквально не завершатся на разумном горизонте времени. Та же причина,
// по которой в самом брифе ("Почему n-body не считается") отвергнуты
// исходные 50 000 000 итераций, просто на несколько порядков мягче. Взято
// 200 итераций (измерено: ~6с в release) — заметно тяжелее дымового
// варианта (3 итерации) для профилирования, без риска упереться в
// MAX_SCALE.
