...
#
# ЗАЩИТА ОТ ОПАСНЫХ ДЕЙСТВИЙ. Платформа считает запуск внешней обработки
# потенциально опасным действием и показывает модальное предупреждение —
# прогон при этом ждёт человека с мышью. Ключа командной строки для этого
# нет: гасится либо свойством ПОЛЬЗОВАТЕЛЯ информационной базы, либо
# параметром `DisableUnsafeActionProtection` в conf.cfg установки.
#
# Первый путь здесь НЕ РАБОТАЕТ, и это проверено: конфигурация у нашей
# базы пустая, ролей в ней нет, и платформа отказывается записывать
# первого же пользователя — «после исполнения в списке не осталось бы ни
# одного пользователя с административными правами». Управление
# пользователями к тому же само числится опасным действием, так что и
# подтверждение бы понадобилось.
#
# Остаётся conf.cfg. Файл принадлежит root и лежит вне проекта, поэтому
# правит его ЧЕЛОВЕК, а не этот скрипт:
#
#   ConfLocation из /opt/1cv8/x86_64/<версия>/conf/conf.cfg указывает на
#   каталог с общим conf.cfg (обычно /opt/1cv8/conf/), и параметр
#   дописывается туда.
#
# Пока он не задан, каждый прогон требует подтверждения, а без человека
# упирается в таймаут ниже.
#
...
# для этого не нужна.
# Путь СТАБИЛЬНЫЙ и не должен меняться: он называется в conf.cfg (см.
# «защита от опасных действий» в шапке файла), а список там задаётся
# путями к базам. Переопределять ONEC_IB можно, но тогда предупреждение
# платформы вернётся.
...
if [ "$RC" -ge 124 ]; then
    ...
    echo "Две типовые причины, обе показывают МОДАЛЬНОЕ окно:" >&2
    echo "  1. Необработанное исключение в скрипте — заверните пробы в" >&2
    echo "     Попытка/Исключение." >&2
    echo "  2. Защита от опасных действий не отключена для этой базы —" >&2
    echo "     платформа ждёт подтверждения на запуск внешней обработки." >&2
    echo "     Снимается в conf.cfg установки, см. шапку этого файла." >&2
    exit 1
fi
