// Чтение метаданных на клиенте; изменяется только собственное временное дерево.
Асинх Функция ПробаЧтенияАсинх(ИдЧтения, ПутьЧтения, МетодЧтения, СинхМетодЧтения)
	...
	Попытка
		...
		Если ТипЗнч(РезультатЧтения) = Тип("Дата") Тогда
			ТекстЧтения = Формат(РезультатЧтения, "ДФ=yyyyMMddHHmmss");
		ИначеЕсли ТипЗнч(РезультатЧтения) = Тип("Булево") Тогда
			ТекстЧтения = ?(РезультатЧтения, "1", "0");
		КонецЕсли;
		ВыводЧтения = "ok|" + Строка(ТипЗнч(РезультатЧтения)) + "|value=" + ТекстЧтения + "|sync_equal=" + ?(РезультатЧтения = СинхЧтение, "1", "0");
	Исключение
		ОшибкаЧтения = ИнформацияОбОшибке();
		ВыводЧтения = "error|stage=" + СтадияЧтения + "|" + ОшибкаЧтения.Описание;
		Если ОшибкаЧтения.Причина <> Неопределено Тогда
			ВыводЧтения = ВыводЧтения + "|cause=" + ОшибкаЧтения.Причина.Описание;
		КонецЕсли;
	КонецПопытки;
	Сообщить(ИдЧтения + Символ(9) + "promise=" + ТипОбещанияЧтения + "|" + ВыводЧтения);
