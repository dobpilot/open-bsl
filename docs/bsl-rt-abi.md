...
Резолвер всегда ставит `Field.open = false`, и `Ч.ТипУзла` с полным
реестром компилируется в ЗАКРЫТЫЙ `GetProp`; открытым идут только методы
(`CallObjectMethod`). Быстрый путь свойств поэтому реально работает через
