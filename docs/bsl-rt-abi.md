...
`ЧтениеXML.ТипУзла`/`.Имя` в цикле (через закрытый `GetProp` —
строковый `get_property` на каждый узел; про открытый двойник см.
ПОПРАВКУ в части C), а `WriteText`/`CloseText` ходят
...
в `step_cold`) обслуживает байт-код, где доступ помечен открытым.

ПОПРАВКА (проверено эмиссией 2026-08-21, см. `docs/bsl-rt-legacy.md`):
утверждение «именно она исполняется при компиляции с реестром» неверно.
Резолвер всегда компилировал свойство закрытым, и `Ч.ТипУзла` с полным
реестром даёт `GetProp`; открытым идут только методы
(`CallObjectMethod`). Флаг `open` у `Field`/`AssignField` вместе с
мёртвыми ветвями компилятора снят по шагу 2 плана в
`docs/bsl-rt-legacy.md`. Быстрый путь свойств поэтому реально работает через
закрытую пару, а открытая остаётся для чтения старого байт-кода. На
решение подключить мост во всех четырёх ветках это не влияет — оно как
раз и оказалось единственно верным. Legacy-пара
...
- **e1.** `TypeRef` + `BslValue::Type(TypeRef)` и рябь по ядру:
  `type_of`, равенство/хеш/печать, `table.rs`, проверка промаха в `vstr`.
  **Сделано.** Уточнения против первоначального замысла: `ОписаниеТипов`
  осталось на `Vec<TypeId>` — его строит `Тип("Имя")`, то есть пока
  всегда нативные типы; сериализация `ЗначениеВФайл` и мэппинги XDTO
  берут `TypeRef::native()` и на тип-дескриптор отвечают прежней ошибкой.
  Официальные компоненты по-прежнему объявляют `legacy_type_id`, поэтому
  измеренное равенство `Тип("…") = ТипЗнч(…)` не тронуто, а `ТипЗнч` над
  host-объектом заработал (проверяется примером `counter`).
- **e2.** `LibraryDescriptor.types`; сбор типов реестра при связывании;
  `Тип("Имя")` ищет в них. **Сделано** — но проще, чем задумывалось:
  список типов прогона лёг в `RuntimeShapes` (он и так протянут всюду и
  уже приходит в `call_builtin_fn_ctx`), а не в `CallContext`, так что
  плести новую нитку через VM не понадобилось. Поля имён `type_ru`/
  `type_en`/`type_identifiers` в дескрипторе НЕ добавлены: пока
  официальные компоненты сохраняют `legacy_type_id`, их имена берутся из
  прежних `NAMES`/`IDENTIFIERS`, а host-типу хватает одного `name`.
  Добавить их придётся на шаге `e3`, когда официальные типы поедут с
  `TypeId` на дескрипторы.
- **e3 и e4 — сделано, одним заходом.** Разделять их было нельзя: между
  ними имена типов лежали бы в ДВУХ местах сразу (таблицы ядра и
  дескрипторы), а это ровно та вторая копия, которую запрещает правило
  переиспользования. Перенос сделан скриптом ИЗ самих таблиц
  `NAMES`/`DISPLAY`/`IDENTIFIERS`, поэтому измеренные написания не могли
  разойтись при переписывании: 93 дескриптора получили `type_display` и
  `type_names`, восемь библиотек — списки `types`, а `TypeId` потерял 93
  варианта и 123 строки имён.

  Дескриптор получил ДВА поля вместо задуманных трёх: `type_display`
  (печать) и `type_names` (все прочие имена поиска). Третье — отдельный
  список идентификаторов — не понадобилось: у платформы «представление»
  и «имя в коде» различаются нерегулярно (у потоков одно представление на
  два типа, у модели схемы наоборот), и один плоский список имён поиска
  описывает все случаи, а `answers_to` отвечает на любой из них.
...
### Что часть E чуть не потеряла

Проверка после реализации нашла три вещи — стоит их помнить, потому что
все три одного рода: перенос имён из ядра в дескрипторы забрал с собой
правила, которые в ядре были неявными.

1. **`ОписаниеТипов` перестал понимать компонентные имена.**
   `new_type_description` резолвил имя ЕДИНСТВЕННЫМ путём мимо реестра —
   через `TypeId::lookup`, — и `Новый ОписаниеТипов("ЧтениеJSON")` стал
   ошибкой. Ни одна фикстура не строит описание типов по компонентному
   имени, поэтому конформанс молчал. Починка: `TypeDescription` и типы
   колонок носят `TypeRef`, конструктор ищет там же, где `Тип("Имя")`.
   Квалификаторы и внутренний формат остались нативными — измерено про
   них только это.
2. **Правило сравнения имён раздвоилось.** Нативный `lookup` сворачивал
   имя через `squash` (регистр И пробелы), а `answers_to` — через
   `folded_eq` (только регистр). `Тип("Документ DOM")` с одним пробелом
   перестал находить «Документ  DOM» с двумя. Оба пути сведены на один
   `squash`.
3. **Измеренная асимметрия потоков легла на алфавит.** На общее
   представление «Файловый поток» откликаются ОБА типа, а разрешение
   берёт первое совпадение в списке библиотеки — и правильный ответ
   держался на том, что `FILE_STREAM_TYPE` оказался раньше
   `MEMORY_STREAM_TYPE` по имени статика. Порядок в списке — часть
   контракта; теперь это закреплено тестом.

Общий вывод для будущих переносов: когда таблица ядра уезжает в
дескрипторы, вместе с данными нужно переносить и ПРАВИЛА, которые эта
таблица применяла, — и искать пути, которые ходили мимо нового источника.
...
- **f6 `bsl-xml/dom.rs` и `xsd.rs`** → каталоги `dom/` (9 файлов) и
  `xsd/` (8). **Сделано** — после волны 2: конверсия их поверхностей на
  таблицы добавила дескрипторы поверх файлов, которые и так несли по
  целой подсистеме, и оба перевалили за три тысячи строк. Резать их
  раньше конверсии было нечего: до неё объектный хвост был одним
  `if/else`, а не таблицами.

Итог волны: в компонентных крейтах самый большой файл рабочего кода —
`bsl-xml/src/xpath.rs` (3523; его поверхность мала, а размер — это
разбор выражений XPath одной подсистемой), самый большой файл вообще —
`xdto/tests.rs` (2996 строк корпуса тестов). Пятизначных файлов не
осталось, четырёхзначных с несколькими подсистемами — тоже.
...
2. **Конверсия оставшихся объектов на таблицы.** `bsl-textdoc`,
   `bsl-zip` и `bsl-pdf` — **сделано**: методы и свойства таблицами,
   регистровый баг имён у них ушёл. `bsl-spreadsheet` — **методы
   сделаны**, свойства остались строковыми со свёрткой `folded_eq`:
   их реализуют `get_property`/`set_property` из value-мира крейта,
   которыми пользуются `pdf_layout`, `xlsx` и публичные реэкспорты, —
   перевод этой поверхности на `&dyn ObjectProtocol` сам по себе
   отдельное изменение. `dom`/`xsd`/`xpath` — **сделано**: узел DOM
   (24 метода, 22 свойства), объекты схемы и результат XPath на таблицах.
   Строковыми остались свойства КОМПОНЕНТЫ XSD — их набор разный у
   каждого вида компоненты, и общая таблица обещала бы больше, чем
   отдаёт; свёртка там теперь `folded_eq`. Отдельная донарезка этих трёх
   файлов не понадобилась: после конверсии их объектные хвосты — таблицы
   дескрипторов, а не вторая подсистема в файле.

   Грабля, которую стоит помнить: таблица ЗАСЛОНЯЕТ строковый путь, и
   имя, забытое в таблице, исчезает молча. Четыре поля печати страницы
   PDF пропали на первом заходе; поймала фикстура `pdf-read`. Проверять
   конверсию нужно не только тестами крейта, но и сверкой полного списка
   русских имён до и после.
