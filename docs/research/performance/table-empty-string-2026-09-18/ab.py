"""Чередующийся A/B до и после разделения пустой строки таблиц."""
import csv
import hashlib
...
BINS = {
    "before": ROOT / "target/bmp-buffer-window-after/bsl-cli",
    "after": ROOT / "target/table-empty-string-after/bsl-cli",
}
            ...
            for mode in ("plain", "optimize"):
                for pair in range(1, 4):
                    for side in (("before", "after") if pair % 2 else ("after", "before")):
                        ...
                        command.append(str(ROOT / "target/table-empty-string-after" / f"{scenario}.bsl"))
                        ...
                        if scenario == "table_save_load":
                            saved = ROOT / "target/table-empty-string-after/save.txt"
