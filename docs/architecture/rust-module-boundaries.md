...
Контекст компиляции динамического кода остаётся нейтральным контрактом
`bsl_bytecode::DynamicScope`: `module` различает entry и владельцев каталога,
`program` — статическую область ROOT и номера фрагментов, `chunk` — чанк.
Кэш хоста учитывает все три части. VM сохраняет владельца на любой глубине
вложенного исполнения; номер каталожного модуля не подменяет номер фрагмента.
...
Дополнение для активного файлового плана: связанные таблицы выделены в
приватный Rc<LinkedTables>, отдельно от заимствования RuntimeRegistry.
Это явное изменение владения для динамических кадров между poll,
не часть прежнего механического разбиения модулей. Таблицы и host-возможности
разделяются без копирования; публичная граница VM не расширяется.
[Обоснование и проверка времени жизни](../research/compatibility/dynamic-owned-tables-2026-09-17.md).

### Кэши принадлежат запуску, а не образу

Статический `RunCaches` остаётся во владении `ProgramExecution`.
...
RunDynamic теперь создаёт кадр того же запуска. Его приватный DynamicImage
содержит подготовленный Program, RuntimeShapes, RunCaches и Rc<LinkedTables>;
это объект исполнения, не сериализуемый образ байт-кода. Кадры и дочерние
async-задачи разделяют его Rc. Последний кадр освобождает образ, отдельного
накопительного реестра нет. Состояния модулей и AsyncState остаются общими,
существующие локали фрагмента ссылаются прямо на стек вызывающего. Статические
чанки сохраняют links владельца; импортные имена из текста ещё не передаются.
[Измерение и приёмка](../research/compatibility/dynamic-execution-2026-09-17.md).
