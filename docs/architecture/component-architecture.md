...
этапы и проверки остаются в
[`background-jobs.md`](../archive/plans/background-jobs.md).
Файловая загрузка модулей определена в
[`module-inclusion.md`](../reference/language/module-inclusion.md), а
async/HTTP-фундамент — в [`http-client.md`](../plans/http-client.md). Если
планы расходятся в нижнем ABI модулей, авторитетен план фоновых заданий.
...
Ожидание уточнено по [серверным замерам](../research/compatibility/job-wait-2026-09-08/followup.md),
а не по противоречивым описаниям синтакс-помощника. Новый менеджер ждёт
любого конечного состояния и возвращает обновлённую группу; уже конечное
задание даёт немедленный возврат независимо от свежести входного снимка.
`Queued` и `Running` публично активны: внутренний старт не завершает ожидание.
Старое `ОжидатьЗавершения` — процедура ожидания успеха всех заданий, с ошибкой
при таймауте, сбое или отмене. Публичные Rust `wait_terminal` остаются join-all.
Дробные секунды усекаются до целых миллисекунд перед проверкой знака:
отрицательные миллисекунды снимают ограничение, ноль означает опрос.

Worker проверяет отмену собственного задания в условии helping под локом
реестра и после возврата из драйвера. `HostErrorCode::Canceled` преобразуется
в неловимую отмену VM, поэтому обычный обработчик исключения и код после
ожидания не исполняются. Ожидаемые дочерние задания автоматически не
отменяются и остаются в обслуживании пула. Точная задержка отмены платформы
не воспроизводится — [измерение](../research/compatibility/job-wait-2026-09-08/cancel-parent.md)
доказывает прерывание до окончания ребёнка, а не мгновенную реакцию.
...
| Раздел плана фоновых заданий | Нормативные диаграммы | Покрытие |
|---|---|---|
