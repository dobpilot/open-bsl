...
| `JOB.MODULE.INIT.КАСАНИЕ` | ЗавершеноАварийно, «Ошибка инициализации модуля: …ЗамерыФоновыйСосед…» | «Завершено; сосед инициализирован» | НАМЕРЕННОЕ расхождение: тела общих модулей — расширение open-bsl |
...
- Тело общего модуля платформа принимает при загрузке так же молча и так
  же ломает инициализацию модуля при касании. Тела модулей open-bsl —
  осознанное расширение; проба `JOB.MODULE.INIT.КАСАНИЕ` расходится
  намеренно.
