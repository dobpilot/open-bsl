...
### Открытый вопрос перед 4.2: видимость полей связывания
...
Это требует решения владельца относительно жёсткого запрета на такой
вынос. Варианты без разрешения тоже меняют границы плана: оставить
определения типов в `lib.rs` либо ввести аксессоры и переписать обращения
в горячем ядре. Ни один вариант не выбран. Запрошено узкое исключение
для существующих потребителей этих двух типов без расширения внешнего
Rust API. Стадия 4.2 не начата, поля не раскрыты; последующие шаги не
выполнялись в обход установленного порядка.
