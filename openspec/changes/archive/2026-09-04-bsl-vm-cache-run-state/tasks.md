...
- [x] 1.1 Тест `bsl-vm`: на старте прогона у каждого чанка по ячейке
  обоих кэшей на инструкцию (инвариант, ушедший из `image::verify`).
- [x] 1.2 Тест `bsl-vm`: второй прогон той же `Program` стартует с
  холодным кэшем — прогрев первого ему не виден.
- [x] 1.3 Тест `bsl-vm`: размер ячейки кэша методов зафиксирован в 32
  байта (переносится из `bsl-bytecode` вместе с типом).

## 2. `bsl-bytecode`: чанк без состояния

- [x] 2.1 Удалить поля `prop_cache`/`method_cache`, методы-аксессоры и
  типы ячеек из `chunk.rs`; перенести типы в `bsl-vm`.
- [x] 2.2 `image::finalize`/`finalize_lone_chunk*` больше не заводят и
...
- [x] 2.3 Вычистить сборку ячеек из тестовых обвязок `bundle.rs`,
...
- [x] 3.1 Ввести `RunCaches` (по вектору ячеек на чанк) с конструктором
  `for_program`; владение — `ProgramExecution`, каталожные модули — в
  `attach_catalog` тем же индексом, что `session_modules`.
- [x] 3.2 Протянуть `&RunCaches` через `poll_linked` → `step` →
...
- [x] 3.3 JIT: поле `caches` в `JitCtx`, протяжка в четыре шима свойств
...
- [x] 4.2 `cargo test --workspace`; отдельно —
  `the_jit_agrees_with_the_interpreter_on_every_script` (весь корпус) и
  полный конформанс `bsl-cli`.
- [x] 4.3 Диф `docs/reference/bsl-api/api.md` пуст: поверхность BSL API
  не задета (страховка, генератор не перезапускается по этой правке).
- [x] 4.4 Чередующийся A/B против базового бинарника ветки, условия по
...
- [x] 4.5 `benchmarks/hot-code-diff.sh`: сдвиг `step` ожидаем — записать
  размеры до/после там же.

## 5. Завершение

- [x] 5.1 Влить дельту в `openspec/specs/bytecode-image/spec.md`,
  переместить изменение в
  `openspec/changes/archive/YYYY-MM-DD-bsl-vm-cache-run-state/`.
- [x] 5.2 Отметить стадию 1 в `docs/plans/bsl-vm-refactor.md` сделанной,
