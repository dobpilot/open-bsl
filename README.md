...
conformance-тесты). Замкнутость фасада проверяется сборкой: крейт
`open-bsl-consumer` зависит только от `open-bsl` и пишет свой компонент,
разбирает ошибку по фазам и значение по видам исключительно его именами —
если для этого понадобится имя из внутреннего крейта, он перестанет
собираться.
