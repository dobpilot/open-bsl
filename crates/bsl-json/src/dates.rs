...
///
/// Остаток СЕКУНД отбрасывается, и это ИЗМЕРЕНО, а не упущено. Смещение
/// не обязано быть кратно минуте: до первого перехода база `tzdata`
/// хранит местное солнечное время, и у Москвы это `+02:30:17`. Платформа
/// печатает такую зону как `+02:30`, а в универсальной записи вычитает
/// все 9017 секунд — то есть две записи одного момента у неё РАСХОДЯТСЯ
/// на 17 секунд (строки `1800 со смещением` и `1800 универсальная` в
/// `measure-zone.platform.txt`: `1800-01-01T12:00:00+02:30` и
/// `1800-01-01T09:29:43Z`). Здесь то же самое, потому что совместимость
/// важнее внутренней стройности.
