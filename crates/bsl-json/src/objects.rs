...
fn writer_open_file(
    ...
    // Файловая система — та, что пришла объекту при построении (ABI-G):
    // вызов метода происходит позже, поэтому ФС берётся с получателя, а не
    // из краткоживущего контекста.
    let writer = receiver
        .downcast_ref::<JsonWriterObject>()
