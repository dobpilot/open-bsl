...
/// что зовущий контекст её предоставить не может (у контекста нет
/// `function_caller`, см. `CallContext::with_execution_parts`). Документ без
/// функции восстановления читается и так, а запрошенная функция при `None`
...
// перед сигнатурой: `with_execution_parts` отдаёт вызывающего как `Option`,
// потому что `CallContext` строится двумя способами. Опкод
// `CallComponent` — единственный путь к этим функциям в дереве — всегда
// строит контекст с `function_caller`, так что сегодня приходит `Some`; но
// контекст без вызывающего законен, и для него чтение и запись без
// обратного вызова обязаны работать, а не падать.
