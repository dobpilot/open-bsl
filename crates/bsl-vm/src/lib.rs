...
fn check_program_tables(program: &Program) -> Result<(), RtError> {
    ...
    for shape in &program.shapes {
        ...
        // «2|2» вместо «2|1»). Сравнение точное: разные номера с равными по
        // свёртке написаниями уже отвергнуты проверкой таблицы имён выше.
        if shape
            .names
            .iter()
            .enumerate()
            .any(|(i, id)| shape.names[..i].contains(id))
        {
            return Err(RtError::InvalidBytecode(
