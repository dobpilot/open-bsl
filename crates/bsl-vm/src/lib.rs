...
fn check_control_flow(program: &Program) -> Result<(), RtError> {
    ...
    for chunk in &program.chunks {
        ...
        if runs_dynamic && chunk.local_names.len() != chunk.n_locals as usize {
            return Err(RtError::InvalidBytecode(
...
fn check_program_tables(program: &Program) -> Result<(), RtError> {
    ...
    // Имя процедуры или функции — ключ вызова ПО ИМЕНИ
    // (`call_module_function`, обратный вызов компонента), а номер `Call` —
    // ключ вызова статического. Резолвер запрещает повтор в исходнике; в
    // образе повтор развёл бы эти два ключа: по имени нашлась бы первая
    // функция, по номеру исполнилась бы вторая.
    for (i, name) in program.function_names.iter().enumerate() {
        if program.function_names[..i]
            .iter()
            .any(|earlier| bsl_rt::folded_eq(earlier, name))
        {
            return Err(RtError::InvalidBytecode(
                "имя процедуры или функции встречается в таблице дважды",
            ));
        }
    }
    Ok(())
