...
fn compile_instr(
    ...
    match *instr {
        ...
        // `Сообщить` обязан писать в поток конкретного `State`, функции
        // окружения — отвечать из его часов, случайности и аргументов
        // запуска, а файловые — читать и писать через его файловую
        // систему. Ничего этого у JIT-шима нет: он работает с
        // sink-потоками и без окружения, поэтому такие функции остаются
        // интерпретатору. Спрашиваем ровно ту классификацию, по которой
        ...
        // шима стоила бы `empty_for` 58 -> 83 млн — см.
        // `docs/archive/refactors/component-abi-f.md`):
