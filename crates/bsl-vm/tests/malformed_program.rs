...
/// Геометрия кадра упорядочена: параметры — первые локали, локали — первые
/// регистры. Границей для `byref:slot` служит `n_locals`, поэтому раздутое
/// `locals` пропускает слот из промежутка `[n_regs, n_locals)`, а
/// `Frame::reg_index` превращает его в индекс ВЫШЕ вершины кадра вызывающего:
/// параметр по ссылке алиасит собственный регистр ВЫЗВАННОЙ функции. До
/// проверки образ печатал 777 вместо 99 и оставлял переменную вызывающего
/// нетронутой — с кодом успеха.
#[test]
fn a_frame_whose_locals_outnumber_its_registers_is_invalid_bytecode() {
    const SRC: &str = concat!(
        "Процедура Уст(Знач З, П)\n",
        "П = З;\n",
        "Врем = 777;\n",
        "КонецПроцедуры\n",
        "А = 1; Б = 2; В = 3;\n",
    ...
    // Контроль: по ссылке записывается 99 в переменную вызывающего.
    assert_eq!(
        bsl_vm::run_program(&compile(SRC)).unwrap().to_string(),
        "99"
    );

    let program = tampered_in(SRC, ".chunk 0 params=0", |line| {
        retoken(line, "locals", "20")
    });
    assert_eq!(
        invalid(&program),
...
/// Кадр строится по режимам МЕСТА ВЫЗОВА, а не по `param_by_val` вызываемой
/// функции, поэтому `byref` против объявленного `Знач` молча превращает копию
/// в алиас: вызванная функция пишет в переменную вызывающего там, где
/// исходник этого не позволяет. До проверки образ возвращал 5 вместо 1.
#[test]
fn passing_by_reference_into_a_znach_parameter_is_invalid_bytecode() {
    const SRC: &str = "Процедура Ф(Знач а)\nа = 5;\nКонецПроцедуры\nх = 1;\nФ(х);\nВозврат х;\n";
    // Контроль: `Знач` защищает переменную вызывающего.
    assert_eq!(bsl_vm::run_program(&compile(SRC)).unwrap().to_string(), "1");

    let program = tampered_in(SRC, "    0 [value]", |line| {
        line.replace("[value]", "[byref:0]")
    });
    assert_eq!(
        invalid(&program),
...
/// Номер имени поля у ОТКРЫТОГО (объектного) доступа резолвится только на
/// ветке «не объект», поэтому у натурального получателя висячий номер давал
/// обычное ловимое «нет такого свойства», которое `Попытка` проглатывает.
/// Граница проверяется статически.
#[test]
fn a_property_name_id_past_the_name_table_is_invalid_bytecode() {
    const SRC: &str = "с = Новый Структура(\"а\", 1);\nВозврат с.а;\n";
    assert_eq!(bsl_vm::run_program(&compile(SRC)).unwrap().to_string(), "1");

    let program = tampered_in(SRC, "GetProp", |line| retoken(line, "name", "9999"));
    assert_eq!(
        invalid(&program),
...
/// `chunks[i+1]`), поэтому обе таблицы сверяются глобально, по длине: пары
/// «лишний безымянный чанк» и «имя без тела» ломают соответствие целиком,
/// даже когда на них никто не ссылается. Удаление тела из середины к тому же
/// сдвигает все последующие, и номер, прошедший поштучную проверку, исполнил
/// бы ЧУЖУЮ функцию. До проверки оба образа исполнялись молча и возвращали
/// свой ответ с кодом успеха.
#[test]
fn tables_of_names_and_bodies_that_disagree_in_length_are_invalid_bytecode() {
    ...
    // Лишний безымянный чанк: имя второй функции убрано, её тело и вызов
    // `func=2` остались.
    let nameless = listing
        .replace(".functions 2", ".functions 1")
        .replace("  1 \"Вторая\"  ; .chunk 2\n", "");
    assert_eq!(
        invalid(&parse_program(&nameless).expect("разбор обязан принять правку")),
        "число чанков не равно числу функций плюс чанк верхнего уровня"
    );

    // Имя без тела: у НЕиспользуемой функции убран сам чанк.
    const UNUSED: &str = concat!(
        "Функция Неиспользуемая()\n",
        "Возврат 1;\n",
        "КонецФункции\n",
        "Возврат 7;\n",
    );
    let listing = write_program(&compile(UNUSED), None).expect("печать листинга");
    let body = listing
        .find("\n.chunk 1 ")
        .expect("в листинге обязан быть чанк функции");
    let bodiless = format!("{}\n", &listing[..body]);
    assert_eq!(
        invalid(&parse_program(&bodiless).expect("разбор обязан принять правку")),
        "число чанков не равно числу функций плюс чанк верхнего уровня"
    );
}

/// Таблица имён КАДРА — такая же биекция «имя ↔ слот». Повтор связывает одно
/// написание с двумя слотами: фрагмент `Выполнить` резолвит переменную в
/// один, окружающий код читает другой, и присваивание уходит не в ту
/// переменную. Проверено через `--run-bytecode`: образ отвечал «1|5» вместо
/// «5|2», с кодом успеха.
///
/// Здесь прогон идёт через `run_program`, у которого компилятора фрагментов
/// нет вовсе, — и это удобный КОНТРОЛЬ: связывание happens до первой
/// инструкции, поэтому целый образ доходит до `Выполнить` и падает на
/// отсутствии компилятора (`DynamicError`), а испорченный отвергается РАНЬШЕ,
/// на связывании. Разные ошибки доказывают, что отвергает именно периметр.
#[test]
fn a_frame_name_table_with_a_repeated_name_is_invalid_bytecode() {
    const SRC: &str = concat!(
        "х = 1;\n",
        "у = 2;\n",
        "Выполнить(\"х = 5\");\n",
    ...
    assert!(
        matches!(
            bsl_vm::run_program(&parse_program(&listing).expect("разбор")),
            Err(RtError::DynamicError(_))
        ),
        "целый образ обязан пройти связывание и дойти до «Выполнить»"
    );

    // Второе имя КАДРА получает написание первого. Правится именно
    // `.localnames`, а не `.top-locals`: это разные таблицы.
    let at = listing
        .find("  .localnames 2\n")
        .expect("в листинге обязана быть таблица имён кадра");
    ...
    assert_eq!(
        invalid(&parse_program(&broken).expect("разбор обязан принять правку")),
        "имя локальной переменной кадра встречается дважды"
    );
}

/// Форма — НАБОР имён: два слота под одним именем дают структуру, у которой
/// `Количество()` считает оба, а доступ по имени берёт значение из чужого
/// слота. До проверки образ отвечал «2|2» вместо «2|1».
#[test]
fn a_shape_repeating_a_field_name_is_invalid_bytecode() {
    const SRC: &str = concat!(
        "с = Новый Структура(\"а, б\", 1, 2);\n",
        "Возврат Строка(с.Количество()) + \"|\" + Строка(с.а);\n",
    );
    let listing = write_program(&compile(SRC), None).expect("печать листинга");
    assert_eq!(
        bsl_vm::run_program(&parse_program(&listing).expect("разбор"))
    ...
    let broken = listing.replace("  0 [0 1]", "  0 [0 0]");
    assert_eq!(
        invalid(&parse_program(&broken).expect("разбор обязан принять правку")),
...
/// Таблица имён обязана быть биекцией `NameId <-> написание`: её строит
/// интернер, у которого одно написание даёт один идентификатор. Повтор
/// разводит ключи, которые обязаны совпадать: одно написание начинает
/// адресовать ДВА разных поля структуры. До проверки такой образ отвечал
/// «1|2» на два обращения по одному имени, с кодом успеха.
#[test]
fn a_name_table_with_a_repeated_spelling_is_invalid_bytecode() {
    const SRC: &str = concat!(
        "с = Новый Структура(\"а, б\", 1, 2);\n",
        "Возврат Строка(с.а) + \"|\" + Строка(с.б);\n",
    );
    let listing = write_program(&compile(SRC), None).expect("печать листинга");
    assert_eq!(
        bsl_vm::run_program(&parse_program(&listing).expect("разбор"))
    ...
    // Второе имя таблицы получает написание первого, номера остаются разными.
    let broken = listing.replace("  1 \"б\"", "  1 \"а\"");
    assert_eq!(
        invalid(&parse_program(&broken).expect("разбор обязан принять правку")),
        "таблица имён содержит повтор написания"
    );
}
