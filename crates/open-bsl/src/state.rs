...
impl Execution<'_, '_> {
    /// Ставит крючок отладчика на этот запуск.
    ///
    /// Отладчик — свойство ЗАПУСКА, а не встраивания: хост, поднявший
    /// движок, решает сам, отлаживать ли конкретный прогон.
    ///
    /// Крючок заодно выключает сцепление линейных цепочек бандлов, иначе
    /// остановка приходила бы не на той инструкции, о которой просили.
    pub fn set_debug_hook(&mut self, hook: Box<dyn bsl_vm::DebugHook>) {
