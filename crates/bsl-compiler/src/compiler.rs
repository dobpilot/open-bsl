...
pub enum CompileError {
    /// Запрошены и сведения об отладке, и оптимизация, которая их
    /// обесценивает.
    ///
    /// Таких две, и обе теряют невосстановимое. `copy-elim` удаляет
    /// инструкции, и таблица строк расходится с кодом. `ssa-regalloc`
    /// переставляет локальные слоты, а имена остаются в исходном порядке,
    /// и отладчик показал бы чужое значение — молча и правдоподобно.
    ///
...
impl Optimizations {
    /// Обесценивает ли выбранный набор сведения об отладке.
    ///
    ...
    #[must_use]
    pub fn breaks_debug_info(self) -> bool {
        let Self {
            const_fold,
            ssa_regalloc,
            ssa_const,
        ...
        // Свёртка в кодогене, распространение констант по SSA и позднее
        // распространение над байт-кодом заменяют инструкции, но не
        // переставляют ни их число, ни слоты.
        let _ = (const_fold, ssa_const, const_prop);
        // `copy_propagate` УДАЛЯЕТ инструкции (`chunk.instrs.remove`), и
        // таблица строк расходится с кодом.
        //
        // `ssa_regalloc` переставляет ЛОКАЛЬНЫЕ СЛОТЫ по регистрам кадра,
        // а `local_names` остаются в исходном порядке — отладчик читает
        // имя по его позиции и показал бы чужое значение. И то и другое —
        // невосстановимая потеря: ни строку, ни соответствие имени слоту
        // из байт-кода не вывести.
        copy_elim || ssa_regalloc
    }
...
pub fn compile_snippet_with_requirements(
    ...
    // Фрагмент — одиночный чанк, программы вокруг него нет. Именно
    // `_unbundled`: разметку VLIW-бандлов фрагмента считает не
...
fn compile_chunk(
    ...
    c.patch_gotos()?;
    // Производные таблицы — разметка бандлов и пометка «трогает объекты» —
    // здесь не заполняются вовсе: их ставит `image::finalize` при сборке
    // программы, и с закрытыми полями иначе и не выйдет.
...
impl<'a> Compiler<'a> {
    ...
    fn compile_block(&mut self, stmts: &[RStmt]) -> Result<(), CompileError> {
        // Строка внешнего оператора СОХРАНЯЕТСЯ и возвращается на выходе.
        // Без этого инструкции, которые внешний оператор выпускает ПОСЛЕ
        // своего тела — back-edge цикла, склейки ветвлений `Если`, —
        // получали бы строку последнего вложенного оператора.
        //
        // Это не косметика: у цикла тогда и тело, и back-edge несут одну
        // строку, она перестаёт меняться, и точка останова в теле
        // срабатывает ОДИН раз вместо каждой итерации. Измерено на цикле
        // в пять витков: одна остановка вместо пяти.
        let outer = self.cur_line;
        for s in stmts {
