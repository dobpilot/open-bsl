...
pub fn write_str(out: &mut Vec<u8>, bytes: &[u8]) {
    out.push(b'(');
    for &b in bytes {
        match b {
            b'(' | b')' | b'\\' => {
...
pub fn write_value(out: &mut Vec<u8>, value: &PdfValue) {
    match value {
...
pub fn inflate_with_limit<R: std::io::Read>(reader: R, max_out: usize) -> RtResult<Vec<u8>> {
    let mut out = Vec::new();
    reader
        .take(max_out as u64)
        .read_to_end(&mut out)
