...
//!
//! Диагнозы здесь ОБЩИЕ, а не персональные для каждого опкода, и это
//! осознанная потеря точности формулировки в обмен на единственность
//! источника. Прежде `image` разбирал `AddConst` и оба слитых перехода
//! сам и отвечал «регистр сложения с константой выходит за кадр»; теперь
//! регистры спрашиваются у таблицы эффектов (`addressed_regs`), а номера
//! констант — у `Instr::const_index`, и оба источника исчерпывающи по
//! опкодам. Новый слитый опкод попадёт под проверку сам, а не будет ждать,
//! пока о нём вспомнят здесь.
