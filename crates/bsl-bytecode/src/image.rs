...
pub fn verify(program: &Program) -> Result<(), RtError> {
    ...
    // НАЧАЛО ОБЩИХ ПРОВЕРОК — якорь для `benchmarks/perimeter-cost.sh`.
    // Скрипт вырезает блок между этим якорем и парным ему, чтобы собрать
    // сторону «без общих проверок» и измерить их цену. Якорь нужен потому,
    // что переключателя для этого в продукте нет и быть не должно:
    // выключаемая проверка образа — это способ однажды выключить её
    // случайно.
    //
    // Общие проверки операндов идут ОДНИМ проходом и ПОСЛЕ точных.
    //
    // Порядок значим так же, как у свежести разметки: широкий диагноз,
    // стоя первым, перехватывал бы точные — например «цель перехода за
    // пределами чанка». Один проход — потому что все три спрашивают одну
    // и ту же таблицу эффектов об одной и той же инструкции, и считать её
    // трижды значило бы платить втрое за связывание, которое идёт на
    // каждом запуске программы.
    //
    // Пересечение здесь не нужно ни одному из вопросов: `addressed_regs`
    // от него не зависит по построению, а `ModSet` несёт номер слота, а
    // не его проекцию на регистры. `None` заодно и дешевле — проекция не
    // вставляется вовсе.
    for chunk in &program.chunks {
        let frame = chunk.n_regs as usize;
        for instr in &chunk.instrs {
            // Регистр: `reads`/`writes` для этого вопроса непригодны, они
            // несут ещё насыщение (`RunDynamic` объявляет все 256, потому
            // что видит локали по именам) и проекцию модульных слотов. Ни
            // то ни другое не утверждает, что регистр существует, а
            // `addressed_regs` — только адресованные операнды, и потому
            // граница строгая: сам кадр, без расширений.
            let e = crate::analysis::effects(instr, chunk, None);
            if e.addressed_regs.max().is_some_and(|m| m >= frame) {
                return Err(RtError::InvalidBytecode(
                    "инструкция обращается к регистру за пределами кадра",
                ));
            }

            // Модульный слот: какие инструкции к нему обращаются, знает
            // ЕДИНСТВЕННАЯ таблица эффектов, и второй разбор по опкодам
            // здесь недопустим — он разошёлся бы с нею на первом же новом
            // опкоде, потому что исчерпывающей проверкой сборки прикрыта
            // она, а не он.
            ...
            // Номер константы: таблица эффектов её не моделирует, но
            // заводить ЗДЕСЬ перечисление опкодов значило бы развести
            // вторую классификацию. Спрашивается узкий наблюдатель
            // `Instr::const_index` — исчерпывающий по опкодам и потому
            // защищённый сборкой, один на весь проект.
            if let Some(k) = instr.const_index()
                && k as usize >= chunk.consts.len()
            {
                return Err(RtError::InvalidBytecode(
    ...
    // КОНЕЦ ОБЩИХ ПРОВЕРОК — парный якорь.
...
mod tests {
    ...
    /// Регистр за пределами кадра отвергается ДО первой инструкции.
    ///
    /// Проверяется и то, что проверка не ложна на законном образце:
    /// проекция модульных слотов на регистры кладёт в наборы эффектов
    /// номера, которых в кадре может не быть, и наивная граница отвергла
    /// бы обычную программу.
    #[test]
    fn a_register_outside_the_frame_is_rejected_before_the_first_instruction() {
        let mut p = valid_program();
        p.chunks[1].instrs = vec![
            Instr::Move { dst: 250, src: 0 },
            Instr::Return { src: None },
        ];
        finalize(&mut p);
        assert!(
            verify(&p).is_err(),
            "регистр за пределами кадра прошёл проверку"
        );
    }

    /// Динамический фрагмент адресует свои операнды, и они тоже обязаны
    /// лежать в кадре.
    ///
    /// Контрпример прямой: `RunDynamic` насыщает регистровые наборы,
    /// потому что видит локали по ИМЕНАМ. Пока проверка смотрела на
    /// `reads`/`writes`, такую инструкцию приходилось пропускать целиком —
    /// и `RunDynamic { src: 250 }` при кадре в один регистр проходил
    /// периметр, отказывая уже в VM.
    #[test]
    fn a_dynamic_fragment_addresses_its_own_operands() {
        let mut p = valid_program();
        p.chunks[1].instrs = vec![
            Instr::RunDynamic {
                src: 250,
        ...
        assert!(
            verify(&p).is_err(),
            "фрагмент с операндом за кадром прошёл проверку"
        );
    }

    /// Проекция модульных слотов не легализует настоящий регистр за
    /// кадром.
    ///
    /// Второй контрпример: у нулевого чанка с двумя модульными
    /// переменными проекция клала в наборы регистры `0..2`, и расширенная
    /// граница принимала настоящий `Move { dst: 1 }` при кадре в один
    /// регистр. Адресованные операнды проекции не содержат, поэтому
    /// граница строгая.
    #[test]
    fn the_module_projection_does_not_legalise_a_register_outside_the_frame() {
        let mut p = valid_program();
        p.chunks[0].instrs = vec![Instr::Move { dst: 1, src: 0 }, Instr::Return { src: None }];
        p.chunks[0].consts = Vec::new();
        p.chunks[0].n_regs = 1;
        p.module_vars = vec!["А".to_string(), "Б".to_string()];
        p.exported_module_vars = vec![true, true];
        finalize(&mut p);
        assert!(
            verify(&p).is_err(),
            "проекция модульных слотов легализовала регистр за кадром"
        );
    }
