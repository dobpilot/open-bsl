...
pub fn verify(program: &Program) -> Result<(), RtError> {
    ...
    // пределами чанка». Один проход — потому что все
    // три спрашивают одну и ту же таблицу эффектов об одной и той же
    // инструкции, и считать её трижды значило бы платить втрое за
    // связывание, которое идёт на каждом запуске программы.
    //
