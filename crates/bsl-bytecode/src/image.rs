...
pub fn verify(program: &Program) -> Result<(), RtError> {
    ...
    //
    // Всякий регистр, который инструкция АДРЕСУЕТ, обязан лежать в кадре.
    // VM проверяет это на каждом обращении (`at`), а свойство статическое:
    // отказ приходил на той инструкции, до которой дошли, то есть после
    // того как программа уже что-то напечатала.
    //
    // Спрашивается та же таблица эффектов, что и обо всём остальном, но
    // ОТДЕЛЬНЫЙ её набор. `reads`/`writes` для этого вопроса непригодны:
    // они несут ещё насыщение (`RunDynamic` объявляет все 256 регистров,
    // потому что видит локали по именам) и проекцию модульных слотов на
    // регистры `0..overlap`. Ни то ни другое не утверждает, что регистр
    // существует, а `addressed_regs` — только адресованные операнды, и
    // потому граница здесь строгая: сам кадр, без расширений.
    for chunk in &program.chunks {
        let frame = chunk.n_regs as usize;
        for instr in &chunk.instrs {
            // Пересечение здесь не нужно: адресация от него не зависит.
    ...
    // Номер константы обязан лежать в таблице констант чанка. Спрашивается
    // узкий наблюдатель `Instr::const_index` — исчерпывающий по опкодам и
    // потому защищённый сборкой, — а не второй разбор в этом модуле:
    // таблица эффектов константы не моделирует, но заводить ради этого
    // ЗДЕСЬ перечисление опкодов значило бы развести две классификации.
    //
    // Прежде это был единственный поздний отказ своего класса: образ
    // собирался, проходил периметр и падал уже на исполнении, когда
    // программа успела что-то напечатать.
    for chunk in &program.chunks {
        for instr in &chunk.instrs {
