...
pub fn verify(program: &Program) -> Result<(), RtError> {
    ...
    // спрашивать её приходится ОСТОРОЖНО: регистровые наборы несут не
    // только адресацию. В них попадает ещё две вещи, и ни одна не
    // утверждает, что регистр существует.
    //
    // Первая — насыщение: `RunDynamic` объявляет обращение ко всем 256
    // регистрам разом. Такие инструкции пропускаются по флагу.
    //
    // Вторая — проекция модульных слотов на регистры `0..overlap`, то
    // самое преднамеренно ложное пересечение (см. модульный доклад
    // `analysis`). Из-за неё вызов в чанке с модульными переменными
    // объявляет чтение регистров, которых в кадре может не быть: у
    // динамического фрагмента кадр бывает и в один регистр при двух
    // модульных переменных объемлющей программы. Поэтому граница берётся
    // как больший из кадра и проекции.
    //
    // Ложных срабатываний это не даёт: проекция вся ниже границы,
    // насыщение исключено флагом, а настоящая адресация лежит ниже
    // `n_regs`. Пропускает же оно настоящую адресацию в промежутке
    // `n_regs..overlap` — известный пробел, и закрыть его можно только
    // сняв ложное пересечение, а это отдельная работа с измерением.
    for (index, chunk) in program.chunks.iter().enumerate() {
        let overlap = crate::analysis::module_overlap(index, program.module_vars.len());
        let bound = (chunk.n_regs as usize).max(overlap.unwrap_or(0));
        for instr in &chunk.instrs {
            let e = crate::analysis::effects(instr, chunk, overlap);
            if e.regs_saturated {
                continue;
            }
            for set in [&e.reads, &e.reads_positional, &e.writes] {
                if set.max().is_some_and(|m| m >= bound) {
                    return Err(RtError::InvalidBytecode(
