...
/// Объявленный импорт, включая ещё не использованные статическим кодом
/// символы. Псевдоним принадлежит импортёру, а не целевому модулю.
...
pub enum LinkEntry {
    ...
    /// Метод конкретного типа runtime-компонента. `library` индексирует
    /// `Program::requirements`, имена — общую таблицу `Program::names`.
    ObjectMethod {
        library: u8,
        type_name: u16,
        method: u16,
