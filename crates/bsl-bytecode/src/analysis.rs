...
//!   (она живёт в состоянии запуска VM) конфликтом не считается: своя у
//!   каждого `pc`.
//!
...
impl RegSet {
    ...
    ///
    /// Спрашивать его осмысленно у `Eff::addressed_regs`: только он несёт
    /// адресацию. У `reads`/`writes` максимум ничего не значит — они
    /// вбирают ещё насыщение и проекцию модульных слотов.
...
/// разметка бандлов и пометка «трогает объекты». Прежняя редакция этой
/// строки называла один лишь `bundle::compute`, и внешний вызывающий,
/// последовав ей, оставил бы вторую производную таблицу устаревшей.
///
...
/// ни другие адреса не сдвигаются — этот проход не требует пересчёта `pc`
...
/// удаляет, но он МЕНЯЕТ саму инструкцию, а с нею её эффекты, и разметка
/// бандлов, посчитанная до, делает утверждение о независимости для
/// прежней редакции. Требование
