...
//!   отдельный блок модульных переменных, а не регистры кадра. Пересечение
//!   с регистрами (`module_overlap`) моделирует ХУДШИЙ случай по входам, и
//!   стоит объяснить, отчего входов два. При обычном исполнении блок живёт
//!   своей структурой (`ModuleState` в `bsl-vm`), и пересечения нет вовсе.
//!   Но есть вход, где host зовёт функцию модуля: там блок читается из
//!   первых ячеек стека вызывающего и туда же возвращается, то есть в
//!   чанке верхнего уровня слот `s` и регистр `s` — одна ячейка. Модель
//!   берёт этот случай для обоих, и ошибается она в безопасную сторону:
//!   лишний алиас только ДОБАВЛЯЕТ конфликтов, то есть дробит бандлы и
//!   запрещает оптимизации, но никогда не разрешает лишнего. Снять эту
//!   консервативность — отдельная работа с измерением, а не правка
//!   комментария. `Вызов` может транзитивно прочитать и записать любой
//!   модульный слот.
