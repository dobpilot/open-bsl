...
//!
//! * состояние исполнения, включая инлайн-кэши: сохранять его означало бы
//!   сохранять и формы, на которые он ссылается, ради нулевой выгоды;
...
fn parse_program_body(r: &mut Reader) -> Result<Program> {
    ...
    // Разметка бандлов — производная таблица: из файла не читается, а
    // пересчитывается из уже разобранного. Обязана дать то
    // же, что у компилятора — это держит побайтовый round-trip вместе с
    // пометками `; бандл N` в листинге.
    //
    // «То же, что у компилятора» включает и ОТСУТСТВИЕ разметки: образ со
    // сведениями об отладке компилятор собирает непучкованным, и разбор
    // обязан вернуть его таким же. Иначе сохранённый и перечитанный
    // отладочный образ возвращался бы размеченным — и отладчик на нём
    // снова не смог бы остановиться внутри бандла. Признак тот же, что и
    // везде: непустая таблица строк.
    if program.lines.is_empty() {
        crate::image::finalize(&mut program);
    } else {
        crate::image::finalize_unbundled(&mut program);
    }
    Ok(program)
}
