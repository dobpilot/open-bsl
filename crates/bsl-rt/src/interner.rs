...
/// и обращение по имени попадает не в тот слот.
///
/// Проверка идёт САМИМ интернированием, а не отдельной копией правила
/// свёртки: имена подаются по порядку, и уникальному имени интернер обязан
/// выдать номер, равный его позиции. Первое расхождение и есть повтор — на
/// него [`NameInterner::intern`] вернул номер РАНЕЕ встреченного имени. Так
/// правило остаётся одно; вторая его реализация рано или поздно разошлась бы
/// с первой.
pub fn first_folded_duplicate(names: &[String]) -> Option<usize> {
    let mut interner = NameInterner::new();
    for (i, name) in names.iter().enumerate() {
        if interner.intern(name).index() != i {
            return Some(i);
        }
    }
    None
