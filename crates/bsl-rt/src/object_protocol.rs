...
pub trait ObjectProtocol: fmt::Debug + ObjectDowncast {
    ...
    /// собственному типу.
    ///
    /// Реализация несёт две обязанности, и обе нужны ключу `Соответствия`.
    ///
    /// Первая — согласие с [`ObjectProtocol::display`]: `value_eq(a, b) ==
    /// Some(true)` влечёт `display(a) == display(b)`, потому что `Hash`
    /// хэширует именно `display` у типа с `value_eq`. РАЗЛИЧИЯ у неравных
    /// значений не требуется — коллизия хэша законна.
    ///
    /// Вторая — это ОТНОШЕНИЕ ЭКВИВАЛЕНТНОСТИ, а не просто ответ на вопрос:
    /// для одного дескриптора метод либо всегда `None`, либо всегда `Some`;
    /// `value_eq(a, a) == Some(true)`; ответ симметричен и транзитивен.
    /// Реализовав этот метод, тип задаёт свой `PartialEq`, и несимметричный
    /// или нетранзитивный ответ ломает `Соответствие` так же, как
    /// разъехавшийся с равенством хэш.
