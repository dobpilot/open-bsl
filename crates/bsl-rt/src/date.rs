...
impl BslDate {
    ...
    /// `ДеньНедели(Дата)`.
    ///
    /// НЕ ИЗМЕРЕНО: нумерация. Взято `понедельник = 1 .. воскресенье = 7`
    /// — так это описано в документации 1С и так же считает `НачалоНедели`
    /// ниже; альтернатива (`воскресенье = 1`, как в некоторых СУБД) сдвинула
    /// бы обе функции разом. Замер в README.
    ...
    /// суток сохраняется.
    ///
    /// НЕ ИЗМЕРЕНО: что происходит с днём, которого в целевом месяце нет.
    /// Взято ЗАЖАТИЕ в последний день месяца (31 января + 1 месяц =
    /// 29 февраля в високосный год, 28 в обычный) — это и общепринятое
    /// поведение таких функций, и единственный вариант, при котором
    /// `ДобавитьМесяц` никогда не падает. Альтернатива (ошибка) заметно
    /// изменила бы код, который ходит по месяцам циклом. Замер в README.
    ///
...
///
/// НЕ ИЗМЕРЕНО: печатает ли платформа время всегда или опускает его у даты
/// с нулевым временем суток, и как выглядит пустая дата. Взято «всегда с
/// временем, `дд.ММ.гггг ЧЧ:мм:сс`» — самый предсказуемый вариант, при
/// котором строковое представление обратимо. Замер в README.
pub const DEFAULT_PATTERN: &str = "дд.ММ.гггг ЧЧ:мм:сс";
...
///
/// НЕ ИЗМЕРЕНО: точный набор букв и их значения сверх перечисленных
/// (`К` — квартал, `в` — до/после полудня и т.п.). Реализовано то, что
/// нужно перечисленным в брифе шаблонам.
...
///
/// НЕ ИЗМЕРЕНО целиком: и набор кодов, и то, как каждый из них выглядит в
...
mod tests {
    ...
    fn add_months_clamps_the_day_into_the_target_month() {
        ...
        // (НЕ ИЗМЕРЕНО, см. doc comment — фиксируем выбранное поведение).
    ...
    fn default_display_is_the_documented_default_pattern() {
        ...
        assert_eq!(BslDate::empty().to_string(), "01.01.0001 00:00:00");
    }
}
