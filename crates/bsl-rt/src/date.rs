...
///
/// НЕ ИЗМЕРЕНО(FMT.DATE.DEFAULT): печатает ли платформа время всегда или
/// опускает его у даты с нулевым временем суток, ведёт ли ведущий ноль в
/// часах и как выглядит пустая дата. Взято «всегда с временем,
/// `дд.ММ.гггг ЧЧ:мм:сс`» — самый предсказуемый вариант, при котором
/// строковое представление обратимо.
