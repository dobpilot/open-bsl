...
pub enum BslObject {
    ...
    /// `ОбъектXDTO` — экземпляр типа объекта: модель и номер типа в ней.
    /// Хранилища свойств у него пока нет (см. шапку `xdto.rs`), поэтому
    /// поля ровно те же, что у `XdtoType`, — но вариант отдельный: у
    /// экземпляра другой тип, другое представление и другой набор членов,
    /// чем у типа, из которого он создан.
    XdtoObject(Rc<crate::xdto::XdtoModel>, usize),
