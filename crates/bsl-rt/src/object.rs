...
pub enum BslObject {
    ...
    /// СНИМКИ, а не окна: платформа отдаёт на два обращения к
    /// `ДочерниеУзлы` разные объекты (измерено), дерево после построения
    /// не меняется, и хранить снимок дешевле, чем ссылку на узел с видом
    /// коллекции. Третье поле — документ, по той же причине, что и у
    /// `DomNode`: пустой список тоже обязан помнить, из какого он дерева.
    DomList(
        crate::dom::DomListKind,
        Vec<Rc<crate::dom::DomNode>>,
        Rc<crate::dom::DomNode>,
    ),
