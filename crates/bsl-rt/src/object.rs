...
pub enum BslObject {
    ...
    TypeDescription(Vec<crate::TypeRef>),
    ...
    /// буфер фиксированного размера, поэтому `RefCell<..>`, а не
    ...
    /// Окном владеет внешний `Rc<BslObject>`; отдельный счётчик окна
    /// не нужен. Общие байты срезов удерживаются внутри `BinBufData`.
    BinaryBuffer(RefCell<crate::bindata::BinBufData>),
