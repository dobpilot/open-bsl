...
/// окружение есть не у всякого вызывающего.
///
/// # Errors
///
...
impl BuiltinFn {
    ...
    /// [`call_builtin_files`]).
    ///
    /// `None` — функция считает ответ по одним аргументам.
    #[must_use]
...
impl BuiltinMethod {
    ...
    /// `ДвоичныеДанные`. Узкий файловый вход отличает двоичные данные, а
    /// остальные получатели делегирует обычной диспетчеризации.
    #[must_use]
    pub fn host_effect(self) -> Option<HostEffect> {
        match self {
...
/// уронить `Sqrt` при автономном вызове, где окружения нет.
///
