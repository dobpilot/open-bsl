...
pub trait TimeZone {
    ...
    ///     перехода.
    ///
    /// Обе стороны укладываются в один порядок проб: сперва решение,
    /// найденное «на месте» (местные секунды как секунды UTC), затем —
    /// смещение заведомо до перехода; оно же остаётся ответом, когда
    /// согласованного решения нет. Остальные десять точек однозначны и
    /// проходят первой пробой.
    ///
    ...
    fn offset_for_local(&self, wall_seconds: i64) -> i32 {
        // Первый кандидат — смещение в момент, если считать показания
        // часов секундами UTC. Годится, только если сам себя подтверждает.
        let here = self.offset_seconds(wall_seconds);
        if self.offset_seconds(wall_seconds - i64::from(here)) == here {
            return here;
        }
        // Второй — заведомо по ту сторону перехода: две смены зоны ближе
        // суток друг к другу база `tzdata` не знает, а любое смещение по
        // модулю меньше 14 часов.
        let before = self.offset_seconds(wall_seconds - 25 * 3600);
        if self.offset_seconds(wall_seconds - i64::from(before)) == before {
            return before;
        }
...
mod tests {
    ...
    fn a_local_time_around_a_transition_resolves_the_measured_way() {
        ...
        // Неподвижная зона отвечает собой в любой момент.
        let fixed = FixedTimeZone::new(-5 * 3600 - 1800).expect("допустимое смещение");
        for wall in [0, 12 * 3600, -3 * 86400, 5 * 86400] {
