...
pub trait TimeZone {
    ...
    fn offset_for_local(&self, wall_seconds: i64) -> i32 {
        ...
        // в пространстве МОМЕНТОВ, а не по местным часам: смещение по
        // модулю меньше 14 часов, значит переход рядом с `wall_seconds`
        // лежит внутри ±14 часов от него, и пробы на ±25 часах дают обе
        // стороны. Пробовать «на месте» (`offset_seconds(wall_seconds)`)
        // нельзя: у западной зоны показания часов сдвинуты в другую
        // сторону, и такая проба берёт кандидата не с той стороны — при
        // переходе `-04:00 -> -05:00` местные 08:30 получали бы `-04:00`,
        // хотя это смещение само себя не подтверждает.
        let probe = |shift: i64| self.offset_seconds(wall_seconds.saturating_add(shift));
        let after = probe(25 * 3600);
        let before = probe(-25 * 3600);
