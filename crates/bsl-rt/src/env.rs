...
/// `Новый ДвоичныеДанные`). Осталось двадцать три обращения, видимых из
/// BSL:
///
///   * ФАЙЛ ЦЕЛИКОМ — четырнадцать: `bsl-json` (`ОткрытьФайл`, `Закрыть`),
///     `bsl-pdf` (чтение и запись), `bsl-spreadsheet` (то же),
///     `bsl-textdoc` (`Прочитать`, `Записать`), `bsl-xml`
///     (`СоздатьФабрикуXDTO`, `ЧтениеXML.ОткрытьФайл`), `bsl-zip`
///     (источник архива, извлечение, добавление, запись);
