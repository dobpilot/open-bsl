...
/// UTF-16 кодовые единицы строки, ОДНОЙ аллокацией.
///
/// Число байт UTF-8 — верхняя граница числа кодовых единиц, и притом
/// точная для ASCII: 1-, 2- и 3-байтовая последовательности дают по одной
/// единице, 4-байтовая — две. Поэтому `with_capacity(s.len())` хватает
/// всегда, и `extend` не перевыделяет ни разу.
///
/// Через `collect()` этого не получить: `EncodeUtf16::size_hint` объявляет
/// нижней границей примерно треть — точнее `len.div_ceil(3)`, — на случай
/// сплошь трёхбайтовых символов, а `collect` резервирует именно по нижней.
/// Строка из ASCII при этом начинала с трети нужного и росла двумя
/// перевыделениями с копированием.
/// Это главный конструктор строк рантайма, так что платил за это каждый
/// `БуферСтрок`, каждый разбор JSON и каждый литерал.
///
/// Плата — память: у кириллицы `s.len()` вдвое больше числа единиц, и
/// ёмкость выходит вдвое больше нужной (прежний путь дорастал примерно до
/// 1,3 от нужного). Обменяно сознательно — см. измерения в истории.
