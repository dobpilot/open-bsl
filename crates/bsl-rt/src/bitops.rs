...
///
/// `НЕ ИЗМЕРЕНО(BIN.RADIX.WIDTH)`: сорок знаков — это всё, что измерено.
/// Потолок длины разбираемой строки, если он у платформы есть, лежит выше и
/// не искался, а вместе с ним неизвестно и поведение за потолком: ошибка или
/// молчаливая потеря старших разрядов. Здесь принимается строка ЛЮБОЙ длины,
/// ограничение — только память; разбор при этом O(n²) по длине, потому что
/// каждый знак умножает накопитель, а `BigInt` внутри [`BslNumber`] растёт
/// вместе со строкой.
fn parse_radix(
    v: &BslValue,
    prefix: &str,
...
mod tests {
    ...
    /// `НЕ ИЗМЕРЕНО(BIN.RADIX.WIDTH)`: фиксирует ВЫБРАННОЕ поведение —
    /// потолка длины у разбора нет. Двести знаков `F` впятеро шире
    /// измеренных сорока: 2^800-1, 241 десятичный знак. Ждём точное
    /// значение, а не только успех: молчаливая потеря старших разрядов
    /// иначе неотличима от разбора.
    #[test]
    fn parse_radix_accepts_strings_wider_than_measured() {
