...
pub const OPEN_QUESTIONS: &[OpenQuestion] = &[
    ...
    // Семь вопросов ниже — про диалект регулярных выражений. Все они
    // заданы ЧЕРЕЗ `Вычислить` внутри `Попытка`: движок уже есть, а
    // поверхности BSL (`СтрНайтиПоРегулярномуВыражению`) ещё нет, поэтому
    // этот интерпретатор на всех семи отвечает «Ошибка», а платформа —
    // по существу. Расхождение на них ОЖИДАЕМО ровно до задачи, которая
    // заводит builtins.
    OpenQuestion {
        id: "REGEX.CLASS.WORD",
        what: "что платформа считает символом слова в `\\w` (и, значит, в \
               `\\b`): только букву, десятичную цифру и подчёркивание или \
               более широкий набор ICU — плюс `\\p{M}` (диакритика), \
               `\\p{Pc}` (соединители) и невидимые U+200C/U+200D",
        chosen: "узкий набор: `[\\p{L}\\p{Nd}_]`. Проба — комбинирующее \
                 ударение U+0301: у ICU оно символ слова, здесь нет",
        blocks: "bsl_rt::regex::PropKind::Word и `is_word_cp` — состав `\\w`, \
                 `\\W` и обеих границ слова",
    },
    OpenQuestion {
        id: "REGEX.CLASS.SPACE",
        what: "что платформа считает пробельным в `\\s`: свойство Unicode \
               `White_Space` или документированный ICU набор \
               `[\\t\\n\\f\\r\\p{Z}]`, в котором нет вертикальной табуляции",
        chosen: "`White_Space` целиком (`char::is_whitespace`), то есть \
                 вертикальная табуляция U+000B — пробельная. Ею проба и \
                 разделяет два набора",
        blocks: "bsl_rt::regex::PropKind::Space — состав `\\s` и `\\S`",
    },
    OpenQuestion {
        id: "REGEX.FOLD.SIMPLE",
        what: "как `(?i)` сворачивает регистр у пар, чьё отображение НЕ \
               одинарное: `ß` против `SS`, `İ` (U+0130) против `i`",
        chosen: "только одинарные отображения `char::to_lowercase`: `ё`/`Ё` \
                 и вся кириллица с латиницей сворачиваются, многосимвольные \
                 пары не сворачиваются вовсе. Брать первый символ \
                 многосимвольного отображения нельзя — это уже другая точка",
        blocks: "bsl_rt::regex::simple_lower и `simple_upper` — сравнение \
                 литералов и классов под `(?i)`",
    },
    OpenQuestion {
        id: "REGEX.FLAG.SCOPE",
        what: "докуда действует инлайн-флаг, встреченный НЕ в начале \
               шаблона: на весь шаблон (включая то, что записано до него) \
               или от своего места до конца охватывающей группы",
        chosen: "от своего места до конца охватывающей группы, как в ICU: в \
                 `а(?i)б` регистр важен для `а` и не важен для `б`, а из \
                 `(?:(?i)а)б` флаг наружу не выходит",
        blocks: "bsl_rt::regex::Parser::inline_flags и `Parser::group` — \
                 сохранение и восстановление флагов на границе группы",
    },
    OpenQuestion {
        id: "REGEX.LINE.TERMINATORS",
        what: "какие символы платформа считает концом строки — для того, что \
               НЕ матчит `.`, и для того, где стоят `^`/`$` под `(?m)`: \
               набор UTS#18 (перевод строки, вертикальная табуляция, перевод \
               формата, возврат каретки, NEL, U+2028, U+2029) или более \
               узкий набор Java без вертикальной табуляции и перевода формата",
        chosen: "набор UTS#18, которому следует ICU. Проба — вертикальная \
                 табуляция между двумя буквами под `.`",
        blocks: "bsl_rt::regex::is_line_terminator — `.`, `^` и `$`",
    },
    OpenQuestion {
        id: "REGEX.ANCHOR.EOL",
        what: "видит ли `$` без `(?m)` ХВОСТОВОЙ перевод строки (то есть \
               находится ли `аб$` в «аб\\n») и открывает ли такой перевод \
               новую строку для `^` под `(?m)`",
        chosen: "как в ICU и Java: `$` без `(?m)` стоит и в самом конце \
                 входа, и перед хвостовым концом строки (`\\r\\n` считается \
                 одним), а `^` под `(?m)` в самом конце входа НЕ стоит — \
                 хвостовой перевод строки новой строки не открывает",
        blocks: "bsl_rt::regex::at_line_end и `at_line_start`",
    },
    OpenQuestion {
        id: "REGEX.REPEAT.EMPTY",
        what: "какая итерация ОГРАНИЧЕННОГО квантора `{n,m}` запоминается \
               группой, когда тело совпало с пустотой. Классические движки \
               дают здесь четыре разных ответа: `python3 re` оставляет текст \
               РАННЕЙ итерации, а последнюю, пустую, ставит правее; `perl` \
               допускает после непустого прохода ещё один пустой и отдаёт \
               группу пустой ещё правее; ECMAScript пустой необязательный \
               проход ОТВЕРГАЕТ, поэтому захват остаётся за проходом, \
               съевшим текст (`0,1` на строках 1, 2 и 6 теста); PCRE2 на \
               тех же строках отвечает `0,1`, на остальных трёх — `0,2`, \
               и с движком не совпадает ни на одной из шести строк",
        chosen: "обрыв на первом пустом необязательном проходе, то есть \
                 ответ `python3 re` — единственного диалекта, с которым \
                 движок совпал на всех шести строках теста (`perl` и \
                 ECMAScript совпадают на трёх, PCRE2 ни на одной). До \
                 появления стража в ограниченной ветви движок отвечал ровно \
                 как PCRE2, так что выбор перенёс его с одного \
                 неизмеренного диалекта на другой",
        blocks: "bsl_rt::regex — `Instr::Guard` в ограниченной ветви \
                 `Compiler::repeat` (блок `min < max`) и тест \
                 an_empty_pass_ends_a_bounded_quantifier_like_the_classics",
    },
];
