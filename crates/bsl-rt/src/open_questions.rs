...
pub const OPEN_QUESTIONS: &[OpenQuestion] = &[
    ...
    OpenQuestion {
        id: "ZIP.WRITER.TAIL",
        what: "какого ТИПА пятый, шестой и седьмой аргументы \
               `Новый ЗаписьФайлаАрхива`: первые четыре измерены (файл или \
               поток, пароль, `ТипФайлаАрхива`, комментарий), восьмого нет \
               вовсе («Конструктор не найден»), а на хвост 8.3.27 отвечает \
               «Несоответствие типов» решительно на всё, что удалось туда \
               подставить — строку, число, дату, булево, массив и члена \
               каждого из шести перечислений ZIP",
        chosen: "хвост принимается только пустым: непустой аргумент на этих \
                 местах — ошибка типа. Отказ здесь честнее догадки, потому что \
                 любой домысленный тип означал бы МОЛЧАЛИВОЕ игнорирование \
                 настройки, которую платформа явно чем-то считает",
        blocks: "bsl-rt/src/zip.rs::configure, ветка `ArchiveKind::Archive`; \
                 расширение фикстуры zip-write на эти три аргумента",
    },
    OpenQuestion {
