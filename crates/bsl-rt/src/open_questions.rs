...
pub const OPEN_QUESTIONS: &[OpenQuestion] = &[
    ...
    OpenQuestion {
        ...
        what: "что отвечает `ЗначениеЗаполнено` от `ЗначениеXDTO` — ошибку, \
               как у типа, свойства и фасета, или «Да»/«Нет», как у обеих \
               коллекций модели типов",
        chosen: "ошибка, по аналогии с четырьмя соседями по семейству: \
                 ИЗМЕРЕНО, что тип объекта, тип значения, свойство и фасет \
                 ...
                 `Попытка`), то есть уносит весь сеанс замеров. Вместо него в \
                 скрипте стоит суррогат: перепроверка измеренного соседа — \
                 `ЗначениеЗаполнено` от типа значения глобальной `ФабрикаXDTO`",
        blocks: "bsl-rt/src/lib.rs::BslValue::is_filled, ветка \
                 `BslObject::XdtoValue`; строка `зап ...` в \
                 tests/conformance/measure/measure-xdto.bsl, которую нельзя \
                 расширить на само значение",
    },
];
