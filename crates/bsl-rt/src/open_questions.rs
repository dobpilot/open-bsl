...
pub const OPEN_QUESTIONS: &[OpenQuestion] = &[
    OpenQuestion {
        id: "STREAM.TEMP_EDGES",
        what: "предельные параметры CreateTempFile, прочие пути FileName и точные ошибки вне измеренных случаев",
        chosen: "параметры — строгий неотрицательный u64, имя файлового потока — исходная строка; компонент не откладывает создание или запись в собственном буфере",
        blocks: "полная совместимость временного потока вне file-temp-parameters, file-temp-visibility и file-stream-name",
    },
    OpenQuestion {
        id: "STREAM.RESIZE_EDGES",
        what: "SetSize: предельные размеры, прочие аргументы и точные тексты ошибок",
        chosen: "строгое целое u64; память через try_reserve, файл через host, буфер через существующий срез; пустой буфер отвергает и 0, и 1; reader-прокси меняет исходный поток",
        blocks: "полная совместимость SetSize вне stream-resize-values, stream-resize-buffer и stream-resize-edges",
    },
    OpenQuestion {
        id: "NOTIFY.EDGE_ARGUMENTS",
        what: "служебные/непрозрачные получатели и точные диагностические тексты конструктора оповещения",
        chosen: "имя через общую Строка без trim; измеренные Boolean/Date/Null/Type/UUID/File/Array/Structure/ValueList/BinaryData ведут себя как их Строка; пустое имя отвергается; все пять TextEncoding в роли ошибочного имени подменяются успешным и делают оба модульных свойства нечитаемыми; переданный ошибочный получатель проверяется по подменённому имени и сохраняется для доставки; три чужих enum-типа отвергаются; UUID и opaque как получатели отвергаются; TypeError и UnknownMethod используют текущие сообщения runtime",
        blocks: "полная совместимость непрозрачных получателей и диагностик ОписаниеОповещения; measure-all требует клиентского контекста для этой пробы",
    },
    OpenQuestion {
        id: "RUNAPP.EDGE_ARGUMENTS",
        what: "прочие типы команды/cwd, пробельные команды, фактический cwd, ошибки spawn/wait и отсутствие кода ОС; NUL рабочего каталога и синхронной команды",
        chosen: "строгие строки и булево ожидание; пустая команда без процесса; Unix argv без shell; NotFound при sync-ожидании даёт 127, при async — Undefined; async-команда обрезается до NUL; прочие ошибки и отсутствие кода дают исключение",
        blocks: "полная совместимость за пределами file-runapp-details и file-runapp-async(-wait); cwd передаётся host без fallback, в отличие от измеренного native-успеха true при отсутствующем cwd",
    },
    OpenQuestion {
        id: "FIND.TRAVERSAL",
        what: "прочие ошибки обхода, иные формы циклов и особых sfile-имён",
        chosen: "порядок host без сортировки, совпадения уровня перед рекурсией, metadata цели ссылки; NotFound и измеренный Linux ELOOP ссылки раздельно останавливают совпадения и выбор каталогов; у выбранного каталога NotFound/NotADirectory после изменения имени и вложенный PermissionDenied пропускают спуск; поздние/несколько оборванных ссылок проверены file-search-late-links, пробельные имена — file-search-link-spaces, ошибки — file-search-metadata-errors; цикл каталога с длинным именем ссылки сохраняет 42 результата до ELOOP; для физического имени со звёздочкой маска с литералом использует байтовый сопоставитель, чистые ? требуют точной длины не меньше трёх, а wildcard-маски без литерала не совпадают; физический каталог `a*` и ссылки с этим именем на каталог, файл или отсутствующую цель не обходятся рекурсивно; удаление сохраняет физический путь; прочие ошибки передаются, защитные пределы глубины/числа элементов остаются политикой open-bsl",
        blocks: "полная совместимость НайтиФайлы для иных sfile, форм циклов и ошибок host",
    },
    OpenQuestion {
        id: "FILE.TIME_EDGE_BEHAVIOR",
        what: "полный диапазон переполнения UTC-времени, результат Date с полями 0-0-0, перенос правила пропуска зоны и строки вне измеренной матрицы",
        chosen: "UTC-счётчик с шагом 1/10000 переполняется как u64 и сохраняется как Date с нулевыми полями; sync/Async/Begin, арифметика и сериализация измерены; пропуск местного времени даёт mtime -1; файловые строки прекращаются по NUL, снимают шесть измеренных краевых пробелов, принимают 8/14 цифр и обе нулевые формы",
        blocks: "полная совместимость строкового представления граничных дат, строк вне file-time-string-arguments и диапазона четырёх методов времени Файл",
    },
    OpenQuestion {
        id: "FILE.EDGE_BEHAVIOR",
        what: "граничные пути, типы аргументов конструктора и ошибки доступа объекта Файл",
        chosen: "путь нормализуется лексически без абсолютизации; необязательный аргумент и одиннадцать дополнительных объектных типов преобразуются общей Строка, пустой СписокЗначений даёт пустой путь, лишние родители отбрасываются; пустая/точечные формы дают пустые свойства, а /, // и /// сохраняются; операционный путь Файл, двоичных и файловых потоков, ЗаписьТекста, JSON/XML-файлов, текстового и табличного документов, ZIP-архивов, файловой фабрики XDTO и двух TLS-сертификатов обрезается по NUL и удаляет измеренные конечные пробельные символы до host; ДокументPDF снимает те же пробелы, но NUL обрезает только при чтении, а запись отказывает; Linux FIFO, Unix-сокет и символьное устройство считаются файлами размера metadata; Существует превращает в Ложь измеренные NotFound, ELOOP, ENOTDIR, ENAMETOOLONG и PermissionDenied",
        blocks: "полная совместимость Файл вне сохранённых oracle: прочие объектные представления, неизмеренные специальные host-объекты, другие I/O-конструкторы, управляющие символы и классы ошибок",
    },
    OpenQuestion {
        id: "FILE.MOBILE_LIBRARY",
        what: "успешные результаты четырёх методов Файл для мобильной библиотеки, тип Картинка и host-граница content:",
        chosen: "RU/EN-имена и измеренные арности опубликованы; стандартный немобильный runtime отказывает непосредственно при вызове без обещания и без host-I/O",
        blocks: "успешный мобильный контекст и полная реализация мобильной части интерфейса Файл",
    },
    OpenQuestion {
        id: "ARRAY.METHOD_INDEX",
        what: "Получить/Установить/Вставить массива: платформенный предел расширения и точные ошибки преобразования индекса",
        chosen: "Number усечён к нулю, Boolean 0/1, String через существующий parse_canonical; дополнительный oracle Get/Set подтвердил пробелы, знак, ведущий ноль, дробь с точкой и отказ запятой, экспоненте и пустой строке; остальные типы отвергаются; [] не меняется",
        blocks: "полная совместимость вне notification-array-access, notification-array-insert и notification-array-index-edges; опасный предел расширения и точные сообщения не измерены",
    },
    OpenQuestion {
        id: "TABLE.METHOD_INDEX",
        what: "Get таблицы/колонок/строки и Set строки: границы диапазона индекса и точные ошибки",
        chosen: "общий канонический разбор подтвердили пробелы, знак, ведущий ноль и дробь с точкой; запятая, экспонента и пустая строка отвергаются; отрицательное значение отвергается до усечения; getter использует прежний индексатор, setter — прежнее приведение и запись ячейки",
        blocks: "полная совместимость вне notification-table-access и notification-table-index-edges; предельные индексы и точные сообщения не измерены, [] и равенство табличных обёрток этим не меняются",
    },
    OpenQuestion {
    ...
    OpenQuestion {
        ...
        what: "что делает `ДобавитьМесяц` с отсутствующим в целевом месяце днём и прочими выходами за календарный диапазон",
        chosen: "зажатие в последний день месяца (31.01 + 1 месяц = 29.02); BSL возвращает пустую дату при выходе за диапазон (измерен +1 месяц из декабря 9999), Rust сохраняет Option; дроби отбрасываются по file-date-components",
    ...
    OpenQuestion {
        ...
        chosen: "`Неопределено`/прежнее значение до terminal. ИЗМЕРЕНО на                  файловой базе (2026-08-27): совпало; остаётся                  клиент-серверное подтверждение",
