...
/// запуска сама с собой. Так уже падало раньше — тогда на `binary-data`;
/// проявляется это от того, насколько сойдутся по времени два обхода,
/// поэтому добавление любой пары фикстур способно опасность разбудить.
///
