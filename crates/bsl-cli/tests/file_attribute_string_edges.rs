//! Сверка строковых аргументов атрибута с серверным oracle 1С.
...
#[test]
fn attribute_strings_match_native_whitespace_and_words_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source = root.join("tests/conformance/measure/filesystem/file-attribute-string-edges.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-attribute-string-edges.platform.txt"
    );
    let expected = canonical_output(oracle);
    assert_eq!(expected.len(), 40, "38 проб и два граничных маркера");
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-attribute-string-edges-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
