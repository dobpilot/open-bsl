...
impl Scratch {
    fn new() -> Self {
        ...
        let path = std::env::temp_dir().join(format!(
            "open-bsl-search-sfile-selection-{}-{nonce}",
            std::process::id()
        ));
...
#[test]
fn sfile_selection_matches_the_server_oracle_in_both_formats() {
    let source = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("../../tests/conformance/measure/filesystem/file-search-sfile-selection.bsl");
    ...
    for optimize in [false, true] {
        for bytecode in [false, true] {
            let listing = scratch.0.join("search-sfile-selection.bslc");
            if bytecode {
