//! Сверка измеренных Linux-путей трёх файловых читателей с серверным oracle 1С.
...
#[test]
fn file_readers_match_native_io_paths_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source = root.join("tests/conformance/measure/filesystem/file-read-io-edges.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-read-io-edges.platform.txt"
    );
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-read-io-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
