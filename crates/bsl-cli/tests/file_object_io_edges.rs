//! Сверка измеренных Linux-путей I/O объекта Файл с серверным oracle 1С.
...
#[test]
fn file_methods_and_binary_write_match_native_io_paths_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source = root.join("tests/conformance/measure/filesystem/file-object-io-edges.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-object-io-edges.platform.txt"
    );
    let expected = canonical_output(oracle);
    assert_eq!(expected.len(), 24, "22 пробы и два граничных маркера");
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-object-io-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
