//! Сверка измеренных Linux-путей СоздатьКаталог с серверным oracle 1С.
...
#[test]
fn create_directory_matches_native_suffix_paths_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source = root.join("tests/conformance/measure/filesystem/file-create-directory-edges.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-create-directory-edges.platform.txt"
    );
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-create-directory-edges-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
