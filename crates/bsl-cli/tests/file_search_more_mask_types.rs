//! Дополнительные объектные типы маски sync/Async по клиентскому oracle 1С.
...
#[test]
fn object_masks_match_sync_and_async_native_results_in_every_execution_mode() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source =
        root.join("tests/conformance/measure/filesystem/file-search-more-mask-types-async.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-search-more-mask-types-async.platform.txt"
    );
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-search-more-mask-types-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
