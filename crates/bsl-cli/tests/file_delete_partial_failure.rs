//! Частичный отказ `УдалитьФайлы` сверяется с серверным oracle 1С.
...
#[test]
fn partial_delete_failure_matches_native_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source = root.join("tests/conformance/measure/filesystem/file-delete-partial-failure.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-delete-partial-failure.platform.txt"
    );
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-delete-partial-failure-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
