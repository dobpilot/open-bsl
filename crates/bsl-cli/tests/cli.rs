...
/// Обработчик `Исключение` обязан видеть значения, какими они были в
/// точке БРОСКА, а не в конце защищённого блока.
///
/// Граф несёт исключительное ребро на уровне блока, и SSA передаёт
/// обработчику состояние конца блока — значит `Мертвая` выглядит мёртвой
/// до конца тела и получает регистр `А`, а подстановка констант считает
/// `А` равной 2. Печатается 3 или 2 вместо 1. Оба потребителя SSA поэтому
/// выключены на чанке с `Попытка`, и держит это исполнительная проверка:
/// форма байт-кода тут ничего не докажет, потому что дефект виден только
/// на броске.
#[test]
fn a_handler_sees_the_values_from_the_point_the_exception_was_raised() {
    let path = std::env::temp_dir().join(format!(
        "bsl-cli-test-handler-state-{}.bsl",
        std::process::id()
    ));
    std::fs::write(
        &path,
    ...
    let plain = stdout_of(&run(&[script]));
    assert_eq!(plain, "1\n", "без оптимизаций обработчик уже видит не то");
    for spec in [
        "--optimize",
        "--optimize=ssa-const",
        "--optimize=ssa-regalloc",
        "--optimize=ssa-const,ssa-regalloc",
    ] {
        assert_eq!(
            stdout_of(&run(&[spec, script])),
            plain,
            "{spec}: обработчик увидел значение, которого в точке броска не было"
        );
    }
