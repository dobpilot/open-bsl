//! Сверка частичных отказов `СоздатьКаталог` с серверным oracle 1С.
...
#[test]
fn partial_directory_creation_failures_match_native_in_every_execution_mode() {
    let root = Path::new(env!("CARGO_MANIFEST_DIR")).join("../..");
    let source =
        root.join("tests/conformance/measure/filesystem/file-create-directory-failures.bsl");
    let oracle = include_str!(
        "../../../tests/conformance/measure/filesystem/file-create-directory-failures.platform.txt"
    );
    let expected = canonical_output(oracle);
    assert_eq!(expected.len(), 4, "две пробы и два граничных маркера");
    ...
    for optimize in [false, true] {
        ...
        let bytecode = std::env::temp_dir().join(format!(
            "open-bsl-file-create-directory-failures-{}-{nonce}.bslc",
            usize::from(optimize)
        ));
