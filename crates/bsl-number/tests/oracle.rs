...
/// НЕ ИЗМЕРЕНО(SQRT.SMALL_ARG) — точнее, измерены обе точки, и они
/// противоречат друг другу. Платформа даёт 0,141421356237309, наша модель
/// "15 значащих с округлением" даёт 0.14142135623731. У sqrt(2) и
/// sqrt(0.02) одинаковая последовательность цифр, и оба f64 округляются
/// ВВЕРХ на 16-м разряде:
...
/// точки. Нужны доизмерения — соседние точки (`Sqrt(200)`, `Sqrt(0.0002)`,
/// `Log`, `Exp`) стоят в скрипте замеров, тест остаётся под `#[ignore]`.
#[test]
#[ignore]
