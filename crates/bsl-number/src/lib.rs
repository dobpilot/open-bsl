...
///
/// НЕ ИЗМЕРЕНО(NUM.ROUND.MODE_CODES), всё в этом типе: ни какими числами
/// платформа кодирует режимы, ни даже то, что вторая схема — именно
/// half-even (у 1С режимы называются `Окр15как10`/`Окр15как20`, и «как10»
/// может оказаться округлением половины ВНИЗ, а не к чётному). Умолчание —
/// отдельный вопрос, `NUM.ROUND.DEFAULT_MODE` ниже.
///
/// Оба замера прогоняются скриптом `tests/conformance/measure/measure-all.bsl`,
/// оба перечислены в `bsl_rt::open_questions`.
///
...
///
/// НЕ ИЗМЕРЕНО(NUM.ROUND.DEFAULT_MODE). Взят half-up — та же схема, что у
/// подтверждённого замером деления; это ПРЕДПОЛОЖЕНИЕ о том, что платформа
/// не разводит две схемы, а не факт. Проверяется замером
/// `Округл(2.5)`/`Округл(3.5)` из скрипта замеров.
