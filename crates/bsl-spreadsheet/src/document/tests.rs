...
/// Этим тестам файловая система не нужна: они строят документ и работают с
/// его моделью, не читая и не записывая файлов. Обёртка отдаёт документу
/// системную ФС, чтобы прежние места вызова компилировались без правок
/// (ABI-G, тот же приём, что в `bsl-stream`/`bsl-zip`).
fn new_document() -> BslValue {
    super::new_document(std::rc::Rc::new(bsl_rt::SystemFileSystem))
}
...
fn begin_row_group_rejects_extra_arguments() {
    ...
    let mut ctx = CallContext::minimal(&mut shapes, fmt);
