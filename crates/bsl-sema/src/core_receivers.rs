...
//! `BuiltinMethod` для него не годится. Но открытая операция измеримо
//! дороже закрытой (диспетчеризация через `step_cold`, генерическая
//! погрузка аргументов вместо специализированных `WriteText`/`CloseText`) —
//! на сценариях с плотными вызовами это десятки процентов.
//!
...
//! в модуле — `Новый T` одного и того же типа из `NEW_TYPES` (и имя не
//! перехвачено конструктором реестра). Анализ консервативен: любой путь,
...
/// Статически известный ядровый тип переменной. Компилятору важно только
/// одно деление: `ЗаписьТекста` адресуют спец-опкоды
/// `WriteText`/`CloseText`, выбираемые по паре «метод, арность» без знания
/// типа, — их можно выпускать только там, где тип доказан; остальным
/// ядровым типам достаточно общего `CallMethod`.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub(crate) enum CoreReceiver {
    /// `Новый ЗаписьТекста` — единственный адресат `WriteText`/`CloseText`.
    TextWriter,
    /// Остальные конструкторы из `NEW_TYPES`.
    Other,
}
...
impl Collector<'_> {
    ...
    /// `Новый T(...)` ядрового типа? Реестр опрашивается первым — как в
    /// `resolve_new`: конструктор компонента с ядровым именем (гипотеза,
    /// официальные компоненты так не делают) перехватил бы имя.
    fn classify_ctor(&self, value: &Expr) -> Option<CoreReceiver> {
        let Expr::New { type_name, .. } = value else {
            return None;
        };
        if let Some(registry) = self.registry
            && registry.lookup_constructor(type_name).is_some()
        {
            return None;
        }
        ...
        Some(if matches!(upper.as_str(), "ЗАПИСЬТЕКСТА" | "TEXTWRITER") {
            CoreReceiver::TextWriter
        } else {
            CoreReceiver::Other
        })
    }
