...
function SLAXML:parse(xml,options)
	...
	local function findPI()
		-- ПРАВКА open-bsl: +\128-\255 в классе имени, см. README рядом.
		first, last, match1, match2 = find( xml, '^<%?([:%a_\128-\255][:%w_.\128-\255-]*) ?(.-)%?>', pos )
		if first then
			finishText()
	...
	local function startElement()
		anyElement = true
		-- ПРАВКА open-bsl: +\128-\255 в классе имени.
		first, last, match1 = find( xml, '^<([%a_\128-\255][%w_.\128-\255-]*)', pos )
		if first then
			...
			pos = last+1
			-- ПРАВКА open-bsl: +\128-\255 в классе имени.
			first,last,match2 = find(xml, '^:([%a_\128-\255][%w_.\128-\255-]*)', pos )
			if first then
	...
	local function findAttribute()
		-- ПРАВКА open-bsl: +\128-\255 в классе имени.
		first, last, match1 = find( xml, '^%s+([:%a_\128-\255][:%w_.\128-\255-]*)%s*=%s*', pos )
		if first then
			pos2 = last+1
	...
	local function findElementClose()
		-- ПРАВКА open-bsl: +\128-\255 в классе имени.
		first, last, match1, match2 = find( xml, '^</([%a_\128-\255][%w_.\128-\255-]*)%s*>', pos )
		if first then
			nsURI = nil
