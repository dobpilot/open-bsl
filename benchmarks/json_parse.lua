...
-- собирается конкатенацией. Это НЕ вольность: сборка вне замера, а важно
-- лишь то, чтобы текст совпал с тем, что порождает ЗаписьJSON, — иначе
-- рантаймы разбирали бы разный объём. Совпадение проверено побайтово.
local parts = { '[' }
local flag = false
for i = 1, RECORDS do
    if i > 1 then
...
-- --- проверка (вне замера) -------------------------------------------------
-- Только поля из цифр: `#` в Lua меряет БАЙТЫ, а СтрДлина в BSL — код-юниты
-- UTF-16, и на кириллице суммы разошлись бы от разницы кодировок, а не от
-- разницы разбора. Кириллические поля проверяет счётчик заполненности.
local sum = 0
local complete = 0
for _, rec in ipairs(data) do
    local k = rec['контрагент']
    sum = sum + rec['ид'] + rec['вложенное']['число']
        + #k['инн'] + #k['кпп']
        + #k['банковскийСчет']['номер'] + #k['банковскийСчет']['бик']
        + #k['банковскийСчет']['коррсчет']
        + #k['юрАдрес']['индекс'] + #k['фактическийАдрес']['индекс']
    if #k['наименование'] > 0 and #k['юрАдрес']['адрес'] > 0
        and #k['фактическийАдрес']['город'] > 0 and #rec['теги'][2] > 0 then
        complete = complete + 1
    end
end

print(string.format('длина документа (байт): %d, записей: %d, сумма: %d, полных: %d',
    #text, records, sum, complete))
