...
# что `kill -9` по-прежнему оставит каталог и ОДНУ ИЛИ ОБЕ записи: какие
# именно, зависит от момента (измерено: убитый на `git worktree add` —
# одну, убитый на сборке — обе). Убирать тогда руками, теми же тремя
# командами, что печатает `cleanup_failed`; обычного `--force` при этом
# может не хватить по той же причине блокировки:
#
#   git worktree remove -f -f <каталог>/with
#   git worktree remove -f -f <каталог>/without
#   rm -rf <каталог>
trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
