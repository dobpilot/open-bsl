...
# анонимному пулу констант нормализуется до `<anon>` — без смещения И без
# номера записи: и то и другое переставляет любая посторонняя правка.
...
dump() {
    ...
    objdump -d --disassemble="$sym" "$bin" |
        ...
        sed -E 's/\b[0-9a-f]{4,16} </</g
                ...
                # Пул перенумеровывается ЛЮБОЙ посторонней правкой крейта,
                # поэтому номер записи в сравнение не входит — ни со
                # смещением, ни с суффиксом `.llvm`, ни без них. Прежний
                # шаблон требовал сразу обоих хвостов и потому пропускал
                # голое `<anon.HASH.N>`: правка в ДРУГОЙ функции крейта
                # сдвигала номер, и экран поднимал ложную тревогу на
                # неизменившемся коде. Замечено ровно так.
