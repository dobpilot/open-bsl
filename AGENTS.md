...
External dependencies are deliberately few: `bsl-number` uses `num-bigint`/`num-traits`; `bsl-rt` uses `zip` and `flate2` for ZIP containers, XLSX export, and PDF stream compression (the in-tree inflate/deflate/ZIP implementations were retired in favour of these crates); `bsl-cli` uses `rustyline` for raw-mode line editing; dev-only `insta` takes the AST snapshots and `pprof` renders flamegraphs. New external crates need a reason that cannot be met in-tree.
